...
int svoskern_kdb_register(kdbtab_t *cmd)
{
	return kdb_register(cmd);
}
EXPORT_SYMBOL(svoskern_kdb_register);

void svoskern_kdb_unregister(kdbtab_t *cmd)
{
	kdb_unregister(cmd);
}
EXPORT_SYMBOL(svoskern_kdb_unregister);

int svoskern_kdb_printf(const char *fmt, ...)
{
	va_list	ap;
	int 	r;
...
EXPORT_SYMBOL(svoskern_kdb_printf);

void svoskern_kgdb_breakpoint(void)
{
	kgdb_breakpoint();
}
EXPORT_SYMBOL(svoskern_kgdb_breakpoint);

int svoskern_kdbgetularg(const char *arg, unsigned long *value)
{
	return kdbgetularg(arg, value);
}
EXPORT_SYMBOL(svoskern_kdbgetularg);

int svoskern_kdbgetaddrarg(int argc, const char **argv, int*nextarg, unsigned long *value,
			 long *offset, char **name)
{
	return kdbgetaddrarg(argc, argv, nextarg, value, offset, name);
}
EXPORT_SYMBOL(svoskern_kdbgetaddrarg);

void svoskern_kdb_symbol_print(unsigned long addr,  unsigned int punc)
{
...
static kdbtab_t svos_kdb_cmds[] = {
	{.name = "iob", .func = KDB_FUNCTION svKDBio, .usage = "<port> [<value>]", .help = "Read or write the given port",},
	{.name = "iow", .func = KDB_FUNCTION svKDBio, .usage = "<port> [<value>]", .help = "Read or write the given port",},
	{.name = "iol", .func = KDB_FUNCTION svKDBio, .usage = "<port> [<value>]", .help = "Read or write the given port",},
	{.name = "rdmsr", .func = KDB_FUNCTION svKDBrdmsr, .usage = "<msr#>", .help = "Read MSR (UNPROTECTED)",},
	{.name = "wrmsr", .func = KDB_FUNCTION svKDBwrmsr, .usage = "<msr#> <value>", .help = "Write MSR (UNPROTECTED)",},
	{.name = "rdlapic", .func = KDB_FUNCTION svKDBrdlapic, .usage = " ", .help = "local apic dump",},
	{.name = "wrlapic", .func = KDB_FUNCTION svKDBwrlapic, .usage = " ", .help = "write local apic <register> <value>",},
	{.name = "bpc", .func = KDB_FUNCTION kdb_bpC, .usage = "", .help = "Clear All Breakpoint",},
	{.name = "bpd", .func = KDB_FUNCTION kdb_bpD, .usage = "", .help = "Disable All Breakpoint",},
	{.name = "bpe", .func = KDB_FUNCTION kdb_bpE, .usage = "", .help = "Enable All Breakpoint",},
	{.name = "id", .func = KDB_FUNCTION kdb_id, .usage = "<addr>", .help = "disassemble instruction at address", .minlen = 1, .flags = KDB_REPEAT_NO_ARGS,},
	{.name = "ssr", .func = KDB_FUNCTION kdb_ssr, .usage = "", .help = "Single Step With Register Dump", .minlen = 1, .flags = KDB_REPEAT_NO_ARGS,},
	{.name = "ms", .func = KDB_FUNCTION kdb_ms, .usage = "<addr> <pattern> <length>", .help = "Search for <pattern> starting at <address>", .minlen = 1, .flags = KDB_REPEAT_NO_ARGS,},
	{.name = "mf", .func = KDB_FUNCTION kdb_mf,  .usage = "<addr> <pattern> <length>", .help = "Fill memory with <pattern> starting at <address>", .minlen = 1, .flags = KDB_REPEAT_NO_ARGS,},
	{.name = "so", .func = KDB_FUNCTION kdb_so,  .usage = "", .help = "Step over a function call", .minlen = 1, .flags = KDB_REPEAT_NO_ARGS,},
	{.name = "sor", .func = KDB_FUNCTION kdb_so,  .usage = "", .help = "Step over a function call with register dump", .minlen = 1, .flags = KDB_REPEAT_NO_ARGS,},
	{.name = "sout", .func = KDB_FUNCTION kdb_sout,  .usage = "", .help = "Step out of function call", .minlen = 1, .flags = KDB_REPEAT_NO_ARGS,},
	{.name = "soutr", .func = KDB_FUNCTION kdb_sout,  .usage = "", .help = "Step out of function call with register dump", .minlen = 1, .flags = KDB_REPEAT_NO_ARGS,},
	{.name = "cpuid", .func = KDB_FUNCTION kdb_cpuid,  .usage = "", .help = "Display the cpu model, family and features.", .minlen = 1, .flags = KDB_REPEAT_NO_ARGS,},
};

/*
 * svos_cmds_init() - Add SVOS KDB Enhancements
*/
void 
svos_cmds_init(void)
{
	...
	for (iter = 0; iter < (sizeof(svos_kdb_cmds)/sizeof(svos_kdb_cmds[0])); iter++) {
		kdb_register(&svos_kdb_cmds[iter]);
	}
}
