 ...
 * We have one dev file for complete SST information and control same as the
...
/* Defines for IOCTL ISST_IF_GET_PLATFORM_INFO for TPMI support */
#define TPMI_ISST_IF_API_VERSION	0x02
#define TPMI_ISST_IF_DRIVER_VERSION	0x01
#define TPMI_ISST_IF_CMD_LIMIT		1
...
/**
 * struct perf_level -	User to store perf level and mmio offset
...
/**
 * struct tpmi_per_power_domain_info -	User to store per power_domain SST info
 * @package_id:		Package id for this power_domain
 * @power_domain_id:		Die id, Each instance from the TPMI in a package is a power_domain.
 *			For example, instance 0 is power_domain 0, instance 1 is power_domain 1 etc.
 ...
 * @saved_pp_control:	Save SST-PP control information to store restore for suspene/resume
 *
 * This structure is used store complete SST info for a power_domain. This information
 * is used to read/write request for any SST information. Each physical
 * CPU package can have multiple power_domains. Some of the power_domains may not contain any
 * CPUs.
 */
...
/**
 * struct tpmi_sst_struct -	User to store sst info for a power_domain
 * @pkg_id:		Package id for this aux device instance
 ...
 * Each package has a unique PMT device, which enumerated TPMI
 * Each Package will have multiple power_domains. Each power_domain has its own
 * SST information and controls.
 */
...
/**
 * struct tpmi_sst_common_struct -	Place holder for all SST instances
 * @max_index:		Maximum instances currenly present
...
/*
 * Each IOCTL request is processed under this lock. Also for adding
 * removing isst_common.sst_inst.
 */
...
{\
	u64 val, mask;\
	\
	intel_tpmi_readq(power_domain_info->auxdev, power_domain_info->sst_base + power_domain_info->sst_header.cp_offset + (offset), &val);\
...
{\
	...
	intel_tpmi_writeq(power_domain_info->auxdev, val, power_domain_info->sst_base + power_domain_info->sst_header.cp_offset + (offset));\
...
static long isst_if_core_power_state(void __user *argp)
{
	...
	if (core_power.get_set) {
		_write_cp_info("cp_enable", core_power.enable, SST_CP_CONTROL_OFFSET, SST_CP_ENABLE_START, SST_CP_ENABLE_WIDTH, SST_MUL_FACTOR_NONE)
		_write_cp_info("cp_prio_type", core_power.priority_type, SST_CP_CONTROL_OFFSET, SST_CP_PRIORITY_TYPE_START, SST_CP_PRIORITY_TYPE_WIDTH, SST_MUL_FACTOR_NONE)
	} else {
		/* get */
		_read_cp_info("cp_enable", core_power.enable, SST_CP_STATUS_OFFSET, SST_CP_ENABLE_START, SST_CP_ENABLE_WIDTH, SST_MUL_FACTOR_NONE)
		_read_cp_info("cp_prio_type", core_power.priority_type, SST_CP_STATUS_OFFSET, SST_CP_PRIORITY_TYPE_START, SST_CP_PRIORITY_TYPE_WIDTH, SST_MUL_FACTOR_NONE)
...
static long isst_if_clos_param(void __user *argp)
{
	...
	if (clos_param.get_set) {
		_write_cp_info("clos.min_freq", clos_param.min_freq_mhz, (SST_CLOS_CONFIG_0_OFFSET + clos_param.clos * SST_REG_SIZE), SST_CLOS_CONFIG_MIN_START, SST_CLOS_CONFIG_MIN_WIDTH, SST_MUL_FACTOR_FREQ);
		_write_cp_info("clos.max_freq", clos_param.max_freq_mhz, (SST_CLOS_CONFIG_0_OFFSET + clos_param.clos * SST_REG_SIZE), SST_CLOS_CONFIG_MAX_START, SST_CLOS_CONFIG_MAX_WIDTH, SST_MUL_FACTOR_FREQ);
		_write_cp_info("clos.prio", clos_param.prop_prio, (SST_CLOS_CONFIG_0_OFFSET + clos_param.clos * SST_REG_SIZE), SST_CLOS_CONFIG_PRIO_START, SST_CLOS_CONFIG_PRIO_WIDTH, SST_MUL_FACTOR_NONE);
	} else {
		/* get */
		_read_cp_info("clos.min_freq", clos_param.min_freq_mhz, (SST_CLOS_CONFIG_0_OFFSET + clos_param.clos * SST_REG_SIZE), SST_CLOS_CONFIG_MIN_START, SST_CLOS_CONFIG_MIN_WIDTH, SST_MUL_FACTOR_FREQ)
		_read_cp_info("clos.max_freq", clos_param.max_freq_mhz, (SST_CLOS_CONFIG_0_OFFSET + clos_param.clos * SST_REG_SIZE), SST_CLOS_CONFIG_MAX_START, SST_CLOS_CONFIG_MAX_WIDTH, SST_MUL_FACTOR_FREQ)
		_read_cp_info("clos.prio", clos_param.prop_prio, (SST_CLOS_CONFIG_0_OFFSET + clos_param.clos * SST_REG_SIZE), SST_CLOS_CONFIG_PRIO_START, SST_CLOS_CONFIG_PRIO_WIDTH, SST_MUL_FACTOR_NONE)
...
static long isst_if_clos_assoc(void __user *argp)
{
	...
	for (i = 0; i < assoc_cmds.cmd_count; ++i) {
		...
		offset = SST_CLOS_ASSOC_0_OFFSET + (punit_cpu_no / SST_CLOS_ASSOC_CPUS_PER_REG) * SST_REG_SIZE;
...
{\
	u64 val, _mask;\
	\
	intel_tpmi_readq(power_domain_info->auxdev, power_domain_info->sst_base + power_domain_info->sst_header.pp_offset + (offset), &val);\
...
{\
	...
	val &= ~_mask;\
	val |= (name / div_factor) << start;\
	intel_tpmi_writeq(power_domain_info->auxdev, val, power_domain_info->sst_base + power_domain_info->sst_header.pp_offset + (offset));\
	pr_debug("wr_pp_info %s var:%s wr:%x cp_offset:%x offset:%x start:%x mask:%llx div_factor:%x res:%llx\n",\
...
{\
	u64 val, _mask;\
	\
	intel_tpmi_readq(power_domain_info->auxdev, power_domain_info->sst_base + power_domain_info->perf_levels[level].mmio_offset +\
		(power_domain_info->feature_offsets.bf_offset * 8) + (offset), &val);\
	_mask = GENMASK_ULL((start + width - 1), start);\
	val &= _mask; \
	val >>= start;\
	name = (val * mult_factor);\
...
{\
	...
	\
	intel_tpmi_readq(power_domain_info->auxdev, power_domain_info->sst_base + power_domain_info->perf_levels[level].mmio_offset +\
		(power_domain_info->feature_offsets.tf_offset * 8) + (offset), &val);\
	_mask = GENMASK_ULL((start + width - 1), start);\
	val &= _mask; \
	val >>= start;\
	name = (val * mult_factor);\
...
static int isst_if_get_perf_level(void __user *argp)
{
	...
	_read_pp_info("current_level", perf_level.current_level, SST_PP_STATUS_OFFSET, SST_PP_LEVEL_START, SST_PP_LEVEL_WIDTH, SST_MUL_FACTOR_NONE)
	_read_pp_info("locked", perf_level.locked, SST_PP_STATUS_OFFSET, SST_PP_LOCK_START, SST_PP_LEVEL_WIDTH, SST_MUL_FACTOR_NONE)
	_read_pp_info("feature_state", perf_level.feature_state, SST_PP_STATUS_OFFSET, SST_PP_FEATURE_STATE_START, SST_PP_FEATURE_STATE_WIDTH, SST_MUL_FACTOR_NONE)
	perf_level.enabled = !!(power_domain_info->sst_header.cap_mask & BIT(1));

	_read_bf_level_info("bf_support", perf_level.sst_bf_support, 0 , 0, SST_BF_FEATURE_SUPPORTED_START, SST_BF_FEATURE_SUPPORTED_WIDTH, SST_MUL_FACTOR_NONE);
	_read_tf_level_info("tf_support", perf_level.sst_tf_support, 0 , 0, SST_TF_FEATURE_SUPPORTED_START, SST_TF_FEATURE_SUPPORTED_WIDTH, SST_MUL_FACTOR_NONE);
...
static int isst_if_set_perf_level(void __user *argp)
{
	...
	_write_pp_info("perf_level", perf_level.level, SST_PP_CONTROL_OFFSET, SST_PP_LEVEL_START, SST_PP_LEVEL_WIDTH, SST_MUL_FACTOR_NONE)
...
static int isst_if_set_perf_feature(void __user *argp)
{
	...
	_write_pp_info("perf_feature", perf_feature.feature, SST_PP_CONTROL_OFFSET, SST_PP_FEATURE_STATE_START, SST_PP_FEATURE_STATE_WIDTH, SST_MUL_FACTOR_NONE)
...
{\
	...
	\
	intel_tpmi_readq(power_domain_info->auxdev, power_domain_info->sst_base + power_domain_info->perf_levels[level].mmio_offset +\
		(power_domain_info->feature_offsets.pp_offset * 8) + (offset), &val);\
	_mask = GENMASK_ULL((start + width - 1), start);\
	val &= _mask; \
	val >>= start;\
	name = (val * mult_factor);\
...
static int isst_if_get_perf_level_info(void __user *argp)
{
	...
	_read_pp_level_info("tdp_ratio", perf_level.tdp_ratio, perf_level.level, SST_PP_INFO_0_OFFSET, SST_PP_P1_SSE_START, SST_PP_P1_SSE_WIDTH, SST_MUL_FACTOR_NONE)
	_read_pp_level_info("base_freq_mhz", perf_level.base_freq_mhz, perf_level.level, SST_PP_INFO_0_OFFSET, SST_PP_P1_SSE_START, SST_PP_P1_SSE_WIDTH, SST_MUL_FACTOR_FREQ)
	_read_pp_level_info("base_freq_avx2_mhz", perf_level.base_freq_avx2_mhz, perf_level.level, SST_PP_INFO_0_OFFSET, SST_PP_P1_AVX2_START, SST_PP_P1_AVX2_WIDTH, SST_MUL_FACTOR_FREQ)
	_read_pp_level_info("base_freq_avx512_mhz", perf_level.base_freq_avx512_mhz, perf_level.level, SST_PP_INFO_0_OFFSET, SST_PP_P1_AVX512_START, SST_PP_P1_AVX512_WIDTH, SST_MUL_FACTOR_FREQ)
	_read_pp_level_info("base_freq_amx_mhz", perf_level.base_freq_amx_mhz, perf_level.level, SST_PP_INFO_0_OFFSET, SST_PP_P1_AMX_START, SST_PP_P1_AMX_WIDTH, SST_MUL_FACTOR_FREQ)

	_read_pp_level_info("thermal_design_power_w", perf_level.thermal_design_power_w, perf_level.level, SST_PP_INFO_1_OFFSET, SST_PP_TDP_START, SST_PP_TDP_WIDTH, SST_MUL_FACTOR_NONE)
	perf_level.thermal_design_power_w /= 8; /* units are in 1/8th watt */
	_read_pp_level_info("tjunction_max_c", perf_level.tjunction_max_c, perf_level.level, SST_PP_INFO_1_OFFSET, SST_PP_T_PROCHOT_START, SST_PP_T_PROCHOT_WIDTH, SST_MUL_FACTOR_NONE)
	_read_pp_level_info("max_memory_freq_mhz", perf_level.max_memory_freq_mhz, perf_level.level, SST_PP_INFO_1_OFFSET, SST_PP_MAX_MEMORY_FREQ_START, SST_PP_MAX_MEMORY_FREQ_WIDTH, SST_MUL_FACTOR_FREQ)
	_read_pp_level_info("cooling_type", perf_level.cooling_type, perf_level.level, SST_PP_INFO_1_OFFSET, SST_PP_COOLING_TYPE_START, SST_PP_COOLING_TYPE_WIDTH, SST_MUL_FACTOR_NONE)

	for (i = 0; i < TRL_MAX_LEVELS; ++i) {
		for (j = 0; j < TRL_MAX_BUCKETS; ++j)
			_read_pp_level_info("trl*_bucket*_freq_mhz", perf_level.trl_freq_mhz[i][j], perf_level.level, SST_PP_INFO_4_OFFSET + (i * SST_PP_TRL_0_RATIO_0_WIDTH), j * SST_PP_TRL_0_RATIO_0_WIDTH, SST_PP_TRL_0_RATIO_0_WIDTH, SST_MUL_FACTOR_FREQ);
	}

	for (i = 0; i < TRL_MAX_BUCKETS; ++i)
		_read_pp_level_info("bucket*_core_count", perf_level.bucket_core_counts[i], perf_level.level, SST_PP_INFO_10_OFFSET, SST_PP_TRL_CORES_BUCKET_0_WIDTH * i, SST_PP_TRL_CORES_BUCKET_0_WIDTH, SST_MUL_FACTOR_NONE)

	perf_level.max_buckets = TRL_MAX_BUCKETS;
	perf_level.max_trl_levels = TRL_MAX_LEVELS;

	_read_pp_level_info("p0_freq_mhz", perf_level.p0_freq_mhz, perf_level.level, SST_PP_INFO_11_OFFSET, SST_PP_CORE_RATIO_P0_START, SST_PP_CORE_RATIO_P0_WIDTH, SST_MUL_FACTOR_FREQ)
	_read_pp_level_info("p1_freq_mhz", perf_level.p1_freq_mhz, perf_level.level, SST_PP_INFO_11_OFFSET, SST_PP_CORE_RATIO_P1_START, SST_PP_CORE_RATIO_P1_WIDTH, SST_MUL_FACTOR_FREQ)
	_read_pp_level_info("pn_freq_mhz", perf_level.pn_freq_mhz, perf_level.level, SST_PP_INFO_11_OFFSET, SST_PP_CORE_RATIO_PN_START, SST_PP_CORE_RATIO_PN_WIDTH, SST_MUL_FACTOR_FREQ)
	_read_pp_level_info("pm_freq_mhz", perf_level.pm_freq_mhz, perf_level.level, SST_PP_INFO_11_OFFSET, SST_PP_CORE_RATIO_PM_START, SST_PP_CORE_RATIO_PM_WIDTH, SST_MUL_FACTOR_FREQ)
	_read_pp_level_info("p0_fabric_freq_mhz", perf_level.p0_fabric_freq_mhz, perf_level.level, SST_PP_INFO_11_OFFSET, SST_PP_CORE_RATIO_P0_FABRIC_START, SST_PP_CORE_RATIO_P0_FABRIC_WIDTH, SST_MUL_FACTOR_FREQ)
	_read_pp_level_info("p1_fabric_freq_mhz", perf_level.p1_fabric_freq_mhz, perf_level.level, SST_PP_INFO_11_OFFSET, SST_PP_CORE_RATIO_P1_FABRIC_START, SST_PP_CORE_RATIO_P1_FABRIC_WIDTH, SST_MUL_FACTOR_FREQ)
	_read_pp_level_info("pm_fabric_freq_mhz", perf_level.pm_fabric_freq_mhz, perf_level.level, SST_PP_INFO_11_OFFSET, SST_PP_CORE_RATIO_PM_FABRIC_START, SST_PP_CORE_RATIO_PM_FABRIC_WIDTH, SST_MUL_FACTOR_FREQ)
...
static int isst_if_get_perf_level_mask(void __user *argp)
{
	...
	_read_pp_level_info("count", count, cpumask.level, SST_PP_INFO_1_OFFSET, SST_PP_RSLVD_CORE_COUNT_START, SST_PP_RSLVD_CORE_COUNT_WIDTH, SST_MUL_FACTOR_NONE)
	_read_pp_level_info("mask", mask, cpumask.level, SST_PP_INFO_2_OFFSET, SST_PP_RSLVD_CORE_MASK_START, SST_PP_RSLVD_CORE_MASK_WIDTH, SST_MUL_FACTOR_NONE)
	...
	if (!cpumask.punit_cpu_map) {
		...
		cpumask.cpu_buffer_size = isst_print_cpu_map(cpumask.cpu_buffer,
							     cpumask.cpu_buffer_size, mask,
							     cpumask.socket_id, cpumask.power_domain_id);
	}
...
static int isst_if_get_base_freq_info(void __user *argp)
{
	...
	_read_bf_level_info("p1_high", base_freq.high_base_freq_mhz, base_freq.level, SST_BF_INFO_0_OFFSET, SST_BF_P1_HIGH_START, SST_BF_P1_HIGH_WIDTH, SST_MUL_FACTOR_FREQ)
	_read_bf_level_info("p1_low", base_freq.low_base_freq_mhz, base_freq.level, SST_BF_INFO_0_OFFSET, SST_BF_P1_LOW_START, SST_BF_P1_LOW_WIDTH, SST_MUL_FACTOR_FREQ)
	_read_bf_level_info("BF-TJ", base_freq.tjunction_max_c, base_freq.level, SST_BF_INFO_0_OFFSET, SST_BF_T_PROHOT_START, SST_BF_T_PROHOT_WIDTH, SST_MUL_FACTOR_NONE)
	_read_bf_level_info("BF-tdp", base_freq.thermal_design_power_w, base_freq.level, SST_BF_INFO_0_OFFSET, SST_BF_TDP_START, SST_BF_TDP_WIDTH, SST_MUL_FACTOR_NONE)
...
static int isst_if_get_base_freq_mask(void __user *argp)
{
	...
	_read_bf_level_info("BF-cpumask", mask, cpumask.level, SST_BF_INFO_1_OFFSET, P1_HI_CORE_MASK_START, P1_HI_CORE_MASK_WIDTH, SST_MUL_FACTOR_NONE)
...
static int isst_if_get_turbo_freq_info(void __user *argp)
{
	...
	turbo_freq.max_clip_freqs = SST_TF_MAX_LP_CLIP_RATIOS; /* Restricted set as per documentation */

	for (i = 0; i < turbo_freq.max_clip_freqs; ++i)
		_read_tf_level_info("lp_clip*", turbo_freq.lp_clip_freq_mhz[i], turbo_freq.level, SST_TF_INFO_0_OFFSET, SST_TF_LP_CLIP_RATIO_0_START + (i * SST_TF_LP_CLIP_RATIO_0_WIDTH), SST_TF_LP_CLIP_RATIO_0_WIDTH, SST_MUL_FACTOR_FREQ)

	for (i = 0; i < TRL_MAX_LEVELS; ++i) {
		for (j = 0; j < TRL_MAX_BUCKETS; ++j)
			_read_tf_level_info("cydn*_bucket_*_trl", turbo_freq.trl_freq_mhz[i][j], turbo_freq.level, SST_TF_INFO_2_OFFSET + (i * SST_TF_RATIO_0_WIDTH), j * SST_TF_RATIO_0_WIDTH, SST_TF_RATIO_0_WIDTH, SST_MUL_FACTOR_FREQ)
	}

	for (i = 0; i < TRL_MAX_BUCKETS; ++i)
		_read_tf_level_info("bucket_*_core_count", turbo_freq.bucket_core_counts[i], turbo_freq.level, SST_TF_INFO_1_OFFSET, SST_TF_NUM_CORE_0_WIDTH * i, SST_TF_NUM_CORE_0_WIDTH, SST_MUL_FACTOR_NONE)
...
static int sst_add_perf_profiles(struct auxiliary_device *auxdev, struct tpmi_per_power_domain_info *power_domain_info, int levels)
{
	int i, mask, perf_index;
	u64 perf_level_offsets;

	power_domain_info->perf_levels = devm_kcalloc(&auxdev->dev, levels, sizeof(struct perf_level), GFP_KERNEL);
	if (!power_domain_info->perf_levels)
		return 0;

	power_domain_info->ratio_unit = power_domain_info->pp_header.ratio_unit;
	power_domain_info->avx_levels = SST_MAX_AVX_LEVELS;
	power_domain_info->pp_block_size = power_domain_info->pp_header.block_size;

	/* Read PP Offset 0: Get feature offset with PP level */
	*((u64 *)&power_domain_info->feature_offsets) = readq(power_domain_info->sst_base + power_domain_info->sst_header.pp_offset + 8);
	dev_dbg(&auxdev->dev, "perf-level pp_offset:%x bf_offset:%x tf_offset:%x\n", power_domain_info->feature_offsets.pp_offset,
		power_domain_info->feature_offsets.bf_offset, power_domain_info->feature_offsets.tf_offset);

	perf_level_offsets = readq(power_domain_info->sst_base + power_domain_info->sst_header.pp_offset + 16);
	...
	for (i = 0; i < levels; ++i) {
		...
		offset &= 0xff;
		offset *= 8;
		power_domain_info->perf_levels[i].mmio_offset = power_domain_info->sst_header.pp_offset + offset;
		mask <<= 1;
...
static int sst_main(struct auxiliary_device *auxdev, struct tpmi_per_power_domain_info *power_domain_info)
{
	int i, mask, levels;

	*((u64 *)&power_domain_info->sst_header) = readq(power_domain_info->sst_base);
	power_domain_info->sst_header.cp_offset *= 8;
	power_domain_info->sst_header.pp_offset *= 8;
	dev_dbg(&auxdev->dev, "SST header: interface_ver:0x%x cap_mask:0x%x cp_off:0x%x pp_off:0x%x\n",
		power_domain_info->sst_header.interface_version,
		power_domain_info->sst_header.cap_mask,
		power_domain_info->sst_header.cp_offset,
		power_domain_info->sst_header.pp_offset);

	if (power_domain_info->sst_header.interface_version != ISST_HEADER_VERSION) {
		dev_err(&auxdev->dev, "SST: Unsupported version:%x\n", power_domain_info->sst_header.interface_version);
		return -ENODEV;
	}

	/* Read SST CP Header */
	*((u64 *)&power_domain_info->cp_header) = readq(power_domain_info->sst_base + power_domain_info->sst_header.cp_offset);
	dev_dbg(&auxdev->dev, "CP header: feature_id:0x%x rev:0x%x ratio_unit:0x%x\n",
		power_domain_info->cp_header.feature_id,
		power_domain_info->cp_header.feature_rev,
		power_domain_info->cp_header.ratio_unit
		);

	/* Read PP header */
	*((u64 *)&power_domain_info->pp_header) = readq(power_domain_info->sst_base + power_domain_info->sst_header.pp_offset);
	dev_dbg(&auxdev->dev, "PP header: feature_id:0x%x rev:0x%x level_en_mask:0x%x allowed_lev_mask:0x%x dyn_switch:0x%x ratio_unit:0x%x block_size:0x%x\n",
		power_domain_info->pp_header.feature_id,
		power_domain_info->pp_header.feature_rev,
		power_domain_info->pp_header.level_en_mask,
		power_domain_info->pp_header.allowed_level_mask,
		power_domain_info->pp_header.dynamic_switch,
		power_domain_info->pp_header.ratio_unit,
		power_domain_info->pp_header.block_size
		);

	/* Force level_en_mask level 0 */
	power_domain_info->pp_header.level_en_mask |= 0x01;

	mask = 0x01;
	levels = 0;
	for (i = 0; i < 8; ++i) {
		if (power_domain_info->pp_header.level_en_mask & mask)
			levels = i;
		mask <<= 1;
	}
	power_domain_info->max_level = levels;
	dev_dbg(&auxdev->dev, "max perf level %x\n", levels);
	sst_add_perf_profiles(auxdev, power_domain_info, levels + 1);
...
static int isst_if_mbox_config_tdp_get_levels_info(struct isst_if_mbox_cmd *mbox_cmd)
{
	struct tpmi_per_power_domain_info *power_domain_info;
	u32 resp;
	...
	mbox_cmd->resp_data <<= MBOX_TDP_LEVEL_LOCK_BIT;
	 ...
	 _read_pp_info("locked", resp, SST_PP_STATUS_OFFSET, SST_PP_LOCK_START, SST_PP_LEVEL_WIDTH, SST_MUL_FACTOR_NONE)
	if (resp)
		mbox_cmd->resp_data |= BIT(MBOX_TDP_LEVEL_LOCK_BIT);

	_read_pp_info("current_level", resp, SST_PP_STATUS_OFFSET, SST_PP_LEVEL_START, SST_PP_LEVEL_WIDTH, SST_MUL_FACTOR_NONE)
	mbox_cmd->resp_data |= ((resp & 0xff) << MBOX_CURRENT_CONFIG_TDP_LEVEL_BIT);
	mbox_cmd->resp_data |= ((power_domain_info->max_level & 0xff) << MBOX_CONFIG_TDP_LEVELS_BIT);
...
static int isst_if_mbox_config_tdp_get_tdp_control(struct isst_if_mbox_cmd *mbox_cmd)
{
	struct tpmi_per_power_domain_info *power_domain_info;
	u32 resp;
	...
	_read_bf_level_info("bf_support", resp, 0 , 0, SST_BF_FEATURE_SUPPORTED_START, SST_BF_FEATURE_SUPPORTED_WIDTH, SST_MUL_FACTOR_NONE);
	if (resp)
		mbox_cmd->resp_data |= BIT(MBOX_BF_SUPPORT_BIT);

	_read_tf_level_info("tf_support", resp, 0, 0, SST_TF_FEATURE_SUPPORTED_START, SST_TF_FEATURE_SUPPORTED_WIDTH, SST_MUL_FACTOR_NONE);
	if (resp)
		mbox_cmd->resp_data |= BIT(MBOX_TF_SUPPORT_BIT);

	_read_pp_info("feature_state", resp, SST_PP_STATUS_OFFSET, SST_PP_FEATURE_STATE_START, SST_PP_FEATURE_STATE_WIDTH, SST_MUL_FACTOR_NONE)
	if (resp & BIT(0))
...
static int isst_if_mbox_config_tdp_set_tdp_level(struct isst_if_mbox_cmd *mbox_cmd)
{
	struct tpmi_per_power_domain_info *power_domain_info;
	...
	_write_pp_info("perf_level", mbox_cmd->req_data, SST_PP_CONTROL_OFFSET, SST_PP_LEVEL_START, SST_PP_LEVEL_WIDTH, SST_MUL_FACTOR_NONE)
...
static int isst_if_mbox_config_tdp_set_tdp_control(struct isst_if_mbox_cmd *mbox_cmd)
{
	struct tpmi_per_power_domain_info *power_domain_info;
	int req = 0;
	...
	_write_pp_info("perf_feature", req, SST_PP_CONTROL_OFFSET, SST_PP_FEATURE_STATE_START, SST_PP_FEATURE_STATE_WIDTH, SST_MUL_FACTOR_NONE)
...
static int isst_if_mbox_config_tdp_get_info(struct isst_if_mbox_cmd *mbox_cmd)
{
	struct tpmi_per_power_domain_info *power_domain_info;
	u32 resp;
	...
	_read_pp_level_info("tdp_ratio", resp, mbox_cmd->req_data, SST_PP_INFO_0_OFFSET, SST_PP_P1_SSE_START, SST_PP_P1_SSE_WIDTH, SST_MUL_FACTOR_NONE)
	mbox_cmd->resp_data = resp << MBOX_TDP_RATIO_START;
	_read_pp_level_info("thermal_design_power_w", resp, mbox_cmd->req_data, SST_PP_INFO_1_OFFSET, SST_PP_TDP_START, SST_PP_TDP_WIDTH, SST_MUL_FACTOR_NONE)
...
static int isst_if_mbox_set_pm_config(struct isst_if_mbox_cmd *mbox_cmd)
{
	struct tpmi_per_power_domain_info *power_domain_info;
	u32 req = 0;
	...
	_write_cp_info("cp_enable", req, SST_CP_CONTROL_OFFSET, SST_CP_ENABLE_START, SST_CP_ENABLE_WIDTH, SST_MUL_FACTOR_NONE)
...
static int isst_if_mbox_get_pm_config(struct isst_if_mbox_cmd *mbox_cmd)
{
	struct tpmi_per_power_domain_info *power_domain_info;
	...
	_read_cp_info("cp_enable", resp, SST_CP_STATUS_OFFSET, SST_CP_ENABLE_START, SST_CP_ENABLE_WIDTH, SST_MUL_FACTOR_NONE)
	if (resp)
...
static int isst_if_mbox_get_qos_config(struct isst_if_mbox_cmd *mbox_cmd)
{
	struct tpmi_per_power_domain_info *power_domain_info;
	u32 resp;
	...
	_read_cp_info("cp_prio_type", resp, SST_CP_STATUS_OFFSET, SST_CP_PRIORITY_TYPE_START, SST_CP_PRIORITY_TYPE_WIDTH, SST_MUL_FACTOR_NONE)
	if (resp)
...
static int isst_if_mbox_set_qos_config(struct isst_if_mbox_cmd *mbox_cmd)
{
	struct tpmi_per_power_domain_info *power_domain_info;
	u32 req = 0;
	...
	_write_cp_info("cp_prio_type", req, SST_CP_CONTROL_OFFSET, SST_CP_PRIORITY_TYPE_START, SST_CP_PRIORITY_TYPE_WIDTH, SST_MUL_FACTOR_NONE)
...
static long isst_if_mbox_proc_cmd(u8 *cmd_ptr, int *write_only, int resume)
{
	...
	case CONFIG_TDP_GET_LEVELS_INFO:
		ret = isst_if_mbox_config_tdp_get_levels_info(mbox_cmd);
		*write_only = 0;
		break;
	case CONFIG_TDP_GET_TDP_CONTROL:
		ret = isst_if_mbox_config_tdp_get_tdp_control(mbox_cmd);
		*write_only = 0;
		break;
	case CONFIG_TDP_SET_TDP_CONTROL:
		ret = isst_if_mbox_config_tdp_set_tdp_control(mbox_cmd);
		break;
	case CONFIG_TDP_GET_TDP_INFO:
		ret = isst_if_mbox_config_tdp_get_info(mbox_cmd);
		*write_only = 0;
		break;
	case CONFIG_TDP_SET_LEVEL:
		ret = isst_if_mbox_config_tdp_set_tdp_level(mbox_cmd);
		break;
	case READ_PM_CONFIG:
		ret = isst_if_mbox_get_pm_config(mbox_cmd);
		*write_only = 0;
		break;
	case WRITE_PM_CONFIG:
		ret = isst_if_mbox_set_pm_config(mbox_cmd);
		break;
	case CLOS_PM_QOS_CONFIG:
		if (mbox_cmd->parameter) {
			ret = isst_if_mbox_set_qos_config(mbox_cmd);
		} else {
			ret = isst_if_mbox_get_qos_config(mbox_cmd);
			*write_only = 0;
		}
		break;
	default:
...
static void isst_if_mmio_pm_clos(struct tpmi_per_power_domain_info *power_domain_info, struct isst_if_io_reg *io_reg)
{
	int min, max, prio;
	int clos;
	...
	if (io_reg->read_write) {
		prio = (io_reg->value >> 4) & 0x0f;
		min = (io_reg->value >> 8) & 0xff;
		max = (io_reg->value >> 16) & 0xff;
		pr_debug("set min:%x max:%x prio:%x io_reg:%x\n", min, max, prio, io_reg->value);

		_write_cp_info("clos.min_freq", min, (SST_CLOS_CONFIG_0_OFFSET + clos * SST_REG_SIZE), SST_CLOS_CONFIG_MIN_START, SST_CLOS_CONFIG_MIN_WIDTH, SST_MUL_FACTOR_NONE);
		_write_cp_info("clos.max_freq", max, (SST_CLOS_CONFIG_0_OFFSET + clos * SST_REG_SIZE), SST_CLOS_CONFIG_MAX_START, SST_CLOS_CONFIG_MAX_WIDTH, SST_MUL_FACTOR_NONE);
		_write_cp_info("clos.prio", prio, (SST_CLOS_CONFIG_0_OFFSET + clos * SST_REG_SIZE), SST_CLOS_CONFIG_PRIO_START, SST_CLOS_CONFIG_PRIO_WIDTH, SST_MUL_FACTOR_NONE);
	} else {

		_read_cp_info("clos.min_freq", min, (SST_CLOS_CONFIG_0_OFFSET + clos * SST_REG_SIZE), SST_CLOS_CONFIG_MIN_START, SST_CLOS_CONFIG_MIN_WIDTH, SST_MUL_FACTOR_NONE)
		_read_cp_info("clos.max_freq", max, (SST_CLOS_CONFIG_0_OFFSET + clos * SST_REG_SIZE), SST_CLOS_CONFIG_MAX_START, SST_CLOS_CONFIG_MAX_WIDTH, SST_MUL_FACTOR_NONE)
		_read_cp_info("clos.prio", prio, (SST_CLOS_CONFIG_0_OFFSET + clos * SST_REG_SIZE), SST_CLOS_CONFIG_PRIO_START, SST_CLOS_CONFIG_PRIO_WIDTH, SST_MUL_FACTOR_NONE)
...
static void isst_if_mmio_pm_clos_assoc(struct tpmi_per_power_domain_info *power_domain_info, struct isst_if_io_reg *io_reg)
{
...
static long isst_if_mmio_rd_wr(u8 *cmd_ptr, int *write_only, int resume)
{
	...
	if (io_reg->reg >= PM_CLOS_ASSOC_REG_START) {
...
#define SST_AUTO_SUSPEND_DELAY_MS		2000
...
int tpmi_sst_dev_add(struct auxiliary_device *auxdev)
{
	...
	pm_runtime_set_autosuspend_delay(&auxdev->dev, SST_AUTO_SUSPEND_DELAY_MS);
...
void tpmi_sst_dev_remove(struct auxiliary_device *auxdev)
{
	...
	mutex_lock(&isst_tpmi_dev_lock);
	isst_common.sst_inst[tpmi_sst->pkg_id] = NULL;
	mutex_unlock(&isst_tpmi_dev_lock);
...
void tpmi_sst_dev_suspend(struct auxiliary_device *auxdev)
{
	...
	intel_tpmi_readq(power_domain_info->auxdev, cp_base + SST_CP_CONTROL_OFFSET,
			 &power_domain_info->saved_sst_cp_control);
	...
	intel_tpmi_readq(power_domain_info->auxdev, power_domain_info->sst_base +
							power_domain_info->sst_header.pp_offset + SST_PP_CONTROL_OFFSET,
							&power_domain_info->saved_pp_control);
}
EXPORT_SYMBOL_GPL(tpmi_sst_dev_suspend);
...
void tpmi_sst_dev_resume(struct auxiliary_device *auxdev)
{
	...
	cp_base = power_domain_info->sst_base + power_domain_info->sst_header.cp_offset;
	intel_tpmi_writeq(power_domain_info->auxdev, power_domain_info->saved_sst_cp_control, cp_base + SST_CP_CONTROL_OFFSET);
	...
	intel_tpmi_writeq(power_domain_info->auxdev, power_domain_info->saved_pp_control, power_domain_info->sst_base +
			  power_domain_info->sst_header.pp_offset + SST_PP_CONTROL_OFFSET);
}
EXPORT_SYMBOL_GPL(tpmi_sst_dev_resume);
