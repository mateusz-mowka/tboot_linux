 ...
 * The fabric id, which is a unique number used to represent each
 * cluster. Formula for this is described in the specifications.
 */
...
static int tpmi_uncore_init(struct auxiliary_device *auxdev)
{
	...
	for (i = 0; i < num_resources; ++i) {
		...
		for (j = 0; j < pd_info->cluster_count; ++j) {
			...
			/*
			 * Fabric ID = (power domain id * Num of local fabric clusters in one domain) +
			 * Local fabric cluster ID. As defined in the specifications.
			 */
			cluster_info->uncore_data.fabric_id = j + (i * pd_info->cluster_count);
