...
static ssize_t show_fabric_cluster_id(struct device *dev, struct device_attribute *attr,
				      char *buf)
{
	struct uncore_data *data = container_of(attr, struct uncore_data,
						fabric_cluster_id_dev_attr);
...
int uncore_freq_add_entry(struct uncore_data *data, int cpu)
{
	...
	if (data->domain_id != UNCORE_DOMAIN_ID_INVALID) {
		...
		sprintf(data->name, "package_%02d_die_%02d.%02d", data->package_id,
			data->die_id, ret);
	} else {
