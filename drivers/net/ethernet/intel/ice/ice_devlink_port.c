 ...
 * @new_attr: devlink port attributes requested
 * @extack: extack for reporting error messages
 * @new_port_index: index of newly created devlink port
 *
 * Allocate a new dynamic port instance and prepare it for configuration with
 * devlink.
 *
 * Return: zero on success or an error code on failure.
 */
 ...
 */
static int
ice_dl_port_new_check_attr(struct ice_pf *pf,
			   const struct devlink_port_new_attrs *new_attr,
			   struct netlink_ext_ack *extack)
{
...
/**
 * ice_dl_port_new - devlink handler for the new port
 * @devlink: pointer to devlink
 * @new_attr: pointer to the port new attributes
 * @extack: extack for reporting error messages
 * @new_port_index: pointer to a new port index
 *
		...
		unsigned int *new_port_index)
{
	...
	err = ice_dl_port_new_check_attr(pf, new_attr, extack);
	if (err)
		return err;

	return ice_alloc_dynamic_port(pf, new_attr, extack, new_port_index);
}

/**
 * ice_dl_port_del - devlink handler for port delete
 * @devlink: pointer to devlink
 * @port_index: index of the port to delete
		...
		struct netlink_ext_ack *extack)
{
	...
	dyn_port = xa_load(&pf->dyn_ports, port_index);
	if (!dyn_port) {
...
/**
 * ice_dl_port_fn_hw_addr_get - devlink handler for mac address get
 * @port: pointer to devlink port
 * @hw_addr: hw address to set
 * @hw_addr_len: hw address length
 * @extack: extack for reporting error messages
...
/**
 * ice_dl_port_fn_state_set - devlink handler for port state set
 * @port: pointer to devlink port
 * @state: state to set
 * @extack: extack for reporting error messages
 *
			 ...
			 struct netlink_ext_ack *extack)
{
	...
	/* We shouldn't get here since the only valid cases are ACTIVE or
	 * INACTIVE, and the devlink core checks this.
	 */
	return -EINVAL;
}

/**
 * ice_dl_port_fn_state_get - devlink handler for port state get
 * @port: pointer to devlink port
