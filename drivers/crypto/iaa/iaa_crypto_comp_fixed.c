...
{
	size_t size = sizeof(struct aecs_table_record) + IAA_AECS_ALIGN;
	struct device *dev = &iaa_device->idxd->pdev->dev;
	u32 bfinal = 1;
	u32 offset;

	iaa_device->aecs_table = dma_alloc_coherent(dev, size,
						    &iaa_device->aecs_table_dma_addr, GFP_KERNEL);
	if (!iaa_device->aecs_table)
		return -ENOMEM;

	/* Configure aecs table using fixed Huffman table */
	iaa_device->aecs_table->crc = 0;
	iaa_device->aecs_table->xor_checksum = 0;
	offset = iaa_device->aecs_table->num_output_accum_bits / 8;
	iaa_device->aecs_table->output_accum[offset] = DYNAMIC_HDR | bfinal;
	iaa_device->aecs_table->num_output_accum_bits = DYNAMIC_HDR_SIZE;

	/* Add Huffman table to aecs */
	memcpy(iaa_device->aecs_table->ll_sym, fixed_ll_sym, sizeof(fixed_ll_sym));
	memcpy(iaa_device->aecs_table->d_sym, fixed_d_sym, sizeof(fixed_d_sym));
...
void iaa_aecs_free(struct iaa_device *iaa_device)
{
	...
	dma_free_coherent(dev, size, iaa_device->aecs_table,
			  iaa_device->aecs_table_dma_addr);
}
