	...
	for (i = 0; i < dst_state->num_ldb_ports; i++) {
		...
		if (dlb2_port_find_slot_queue(port, map_state, queue, &slot)) {
			...
			if (infl_cnt < dst_state->ldb_cq_state[vcq].inflights_limit) {
				LM_DEBUG_print("MIG: i = %d, re-enqueue port = %d\n",
						i, port->id.phys_id);
				return port;
			}
		}
		...
		if (dst_queue->sn_cfg_valid == 1 &&
			...
			for (j = 0; j < num_sn; j++) {
				...
				if (dst_state->ldb_qid_state[dst_vqid].sn_state[sn].hl_valid) {
					...
					if (fill_dest_rob_renq(hw, dst_cq, qid2, num_drained, dst_state))
...
bool pf_sch_dummy_hcw(struct dlb2_hw *hw, struct dlb2_ldb_port *dst_port, uint16_t hl_idx,
