 ...
 */
static bool dlb2_is_siov_vdev(struct dlb2 *dlb2)
{
			       ...
			       bool arm)
{
	...
	do {
		...
		/*
		 * The DLB 2.0 Scalable IOV mask/unmask workaround can deliver
		 * spurious CQ interrupts to the VDEV driver. In that case,
		 * wait again. The workaround does not disarm the interrupt, so
		 * no need to re-arm it.
		 */
	} while (ret == 0 && dlb2_is_siov_vdev(dlb2) &&
