...
void add_info_to_devices(struct device_domain_info *info, struct dmar_domain *domain)
{
	unsigned long flags;
...
void del_info_from_devices(struct device_domain_info *info)
{
	unsigned long flags;
...
static void domain_exit(struct dmar_domain *domain)
{
	if (domain->pgd) {
		...
		domain_unmap(domain, 0, DOMAIN_MAX_PFN(domain->gaw), &freelist, NULL);
	...
	case MEM_CANCEL_ONLINE:
		{
			...
			domain_unmap(si_domain, start_vpfn, last_vpfn, &freelist, NULL);
...
static int intel_iommu_hw_info(struct device *dev, struct iommu_hw_info *info)
{
	...
	info->data.vtd.cap_reg &= ~cap_addr_mask;
	info->data.vtd.cap_reg |= ((addr_width & 0x3fULL) << 16);
