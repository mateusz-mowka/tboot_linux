...
static int intel_rapl_tpmi_probe(struct auxiliary_device *auxdev, const struct auxiliary_device_id *id)
{
	...
	while (offset < resource_size(res)) {
		...
		while (++reg_index != TPMI_RAPL_REG_MAX) {
			...
			case TPMI_RAPL_REG_PL1:
				...
				trp->priv.limits[domain_type] |= POWER_LIMIT1;
				break;
			case TPMI_RAPL_REG_PL2:
				reg_id = RAPL_DOMAIN_REG_PL2;
				trp->priv.limits[domain_type] |= POWER_LIMIT2;
				break;
			case TPMI_RAPL_REG_PL4:
				reg_id = RAPL_DOMAIN_REG_PL4;
				trp->priv.limits[domain_type] |= POWER_LIMIT4;
				break;
