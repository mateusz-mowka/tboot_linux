...
static int rapl_config_interface(struct rapl_package *rp)
{
	...
	switch (rp->priv->type) {
	/* PTD I/F shares the same register layout as MSR registers */
	case RAPL_IF_PTD:
	case RAPL_IF_MSR:
...
static int contraint_to_pl(struct rapl_domain *rd, int cid)
{
	...
	for (i = j = POWER_LIMIT1; i < NR_POWER_LIMITS; i++) {
...
static void rapl_init_domains(struct rapl_package *rp)
{
	...
	for (i = 0; i < RAPL_DOMAIN_MAX; i++) {
		...
		for (t = POWER_LIMIT1; t < NR_POWER_LIMITS; t++) {
			if (test_bit(t, &(rp->priv->limits[i])))
 ...
 * The first RAPL domain with no per domain Unit register will use the default
 * Unit register and initialize the default units.
 * Other RAPL Domains within this RAPL package will fallback to the default
 * units if there is no per domain unit register specified.
 */
static int rapl_get_domain_unit(struct rapl_domain *rd)
{
	...
	if (!rd->regs[RAPL_DOMAIN_REG_UNIT]) {
		if (rd->rp->energy_unit)
			goto use_default_unit;

		if (!rd->rp->priv->reg_unit) {
			pr_err("No valid Unit register found\n");
			return -ENODEV;
		}
		/* default units have not been set yet */
		rd->regs[RAPL_DOMAIN_REG_UNIT] = rd->rp->priv->reg_unit;
	}

	if (!rpd->check_unit) {
		pr_err("mandatory .check_unit callback is missing, quit\n");
		return -ENODEV;
	}
	...
	if (!rd->rp->energy_unit && rd->regs[RAPL_DOMAIN_REG_UNIT] == rd->rp->priv->reg_unit) {
		rd->rp->energy_unit = rd->energy_unit;
		rd->rp->power_unit = rd->power_unit;
		rd->rp->time_unit = rd->time_unit;
	}
	return 0;

use_default_unit:
	rd->power_unit = rd->rp->power_unit;
	rd->time_unit = rd->rp->time_unit;
	rd->energy_unit = rd->rp->energy_unit;
