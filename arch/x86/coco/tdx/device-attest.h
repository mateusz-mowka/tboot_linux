...
struct device_info_data {
	...
	struct tpa_spdm_policy          tpa_spdm_policy;
	struct tpa_tdisp_policy         tpa_tdisp_policy;
...
extern bool tdx_attest_device(struct pci_dev *pdev, struct device_info_data *data, size_t data_size,
			      enum attestation_policy policy);

extern bool self_test_device_attestation(struct pci_dev *pdev);
#endif
