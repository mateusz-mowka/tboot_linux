 ...
 * via @err and it may be invalid if the return value is 0.
 */
static int __seamldr_seamcall(u64 fn, u64 rcx, u64 rdx, u64 r8, u64 r9,
			      struct tdx_module_output *out, u64 *err)
{
	int ret;
	u64 vmcs;
	unsigned long flags;
	...
	*err = __seamcall(fn, rcx, rdx, r8, r9, 0, 0, 0, 0, out);

	/* Load the saved VMCS to restore the current-VMCS pointer */
		    ...
		    struct tdx_module_output *out)
{
	...
	if (fn & P_SEAMLDR_SEAMCALL_BASE) {
		err = __seamldr_seamcall(fn, rcx, rdx, r8, r9, out, &ret);
		if (err) {
			return err;
		}
	} else {
 ...
 */
static void e820_next_mem(struct e820_table *table, int *idx, unsigned long *start,
			  unsigned long *end, int *nid_out)
{
	u64 rs, re;
	int rnid, i;
...
again:
	...
	*end = re >> PAGE_SHIFT;
	...
	/*
	 * Always report nid is 0. it may lead to TDMR on one numa node while PAMT on
	 * the other. It doesn't impact functionalities. It is safe because in SVOS
	 * all memory on other numa node may be reserved.
	 */
	if (nid_out)
		*nid_out = 0;
}
 ...
 */
#define _e820_for_each_mem(_table, _i, _start, _end, _nid)			\
	for ((_i) = 0, e820_next_mem((_table), &(_i), (_start), (_end), (_nid));	\
		*(_start) < *(_end);						\
		e820_next_mem((_table), &(_i), (_start), (_end), (_nid)))
 ...
 * Following a successful TDH_SYS_SHUTDOWN, no further TDX module interface
...
static int tdx_prepare_handoff_data(u16 req_hv)
{
	...
	lockdep_assert_held(&tdx_module_lock);
	ret = seamcall(TDH_SYS_SHUTDOWN, req_hv, 0, 0, 0, NULL);
	if (!ret)
		tdx_module_status = TDX_MODULE_SHUTDOWN;
...
int tdx_module_update_prepare(const struct tmu_req *req)
{
	int ret;
	struct seamldr_params *params;
	const struct seam_sigstruct *sig = req->signature;

	params = alloc_seamldr_params(req);
	if (IS_ERR(params))
		return PTR_ERR(params);

	ret = verify_hash(req->module, req->module_size, sig->seamhash);
	if (ret)
		goto free;
	...
	if (req->preserving && !can_preserve_td(req->signature)) {
		ret = -ENODEV;
		goto unlock;
	}
