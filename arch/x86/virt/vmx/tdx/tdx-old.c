...
#define pr_fmt(fmt) "tdx-svos: " fmt
...
static u32 old_tdx_global_keyid __read_mostly;
...
__setup("trace_boot_seamcalls_old", trace_seamcalls);
...
static int build_tdx_memlist(struct list_head *tmb_list)
{
	...
	for (i = 0; i < MAX_CMRS; i++) {
		...
		pr_info("TDMR: adding CMR: [0x%llx, 0x%llx)\n", cmr->base,
				cmr->base + cmr->size);
		 ...
		 */
		start_pfn = max((unsigned long)(cmr->base >> PAGE_SHIFT), PHYS_PFN(SZ_1M));
		end_pfn = (cmr->base + cmr->size) >> PAGE_SHIFT;
		 ...
		 */
		ret = add_tdx_memblock(tmb_list, start_pfn, end_pfn, 0);
		if (ret)
			goto err;
	}
...
static int init_tdx_module(void)
{
	 ...
	 * TDX module metadata.
	 */
	old_tdx_global_keyid = tdx_keyid_start;
 ...
 */
static int old_tdx_enable(void)
{
	int ret;

	if (!old_platform_tdx_enabled()) {
...
static int __init tdx_arch_init(void)
{
	...
	if (tdx_init_rsvd_mem)
		pr_info("using CMRs as TDMRs, thus all reserved memory are included in TDMRs\n");
	else 
		return 0;
