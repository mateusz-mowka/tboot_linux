...
int __init numa_cleanup_meminfo(struct numa_meminfo *mi)
{
	const u64 low = 0;
#ifdef CONFIG_SVOS
	const u64 high = PFN_PHYS(e820__end_of_ram_pfn());
#else
...
#ifdef CONFIG_SVOS
		u64 start = PFN_PHYS(e820__end_of_ram_pfn());
#else
...
static int __init dummy_numa_init(void)
{
	...
	printk(KERN_INFO "Faking a node at [mem %#018Lx-%#018Lx]\n",
	       0LLU, PFN_PHYS(e820__end_of_ram_pfn()) - 1);

	node_set(0, numa_nodes_parsed);
	numa_add_memblk(0, 0, PFN_PHYS(e820__end_of_ram_pfn()));
