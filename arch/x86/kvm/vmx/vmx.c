			     ...
			     struct vmx_capability *vmx_cap)
{
	...
	if (_cpu_based_exec_control & CPU_BASED_ACTIVATE_TERTIARY_CONTROLS) {
		u64 opt3 = TERTIARY_EXEC_IPI_VIRT | TERTIARY_EXEC_MSRLIST;
...
static int handle_help_rdmsrlist(int num, struct kvm_vcpu *vcpu)
{
	...
	for (i = num; i < 64; i++) {
		if (rcx & (1 << i)) {
			p = kvm_read_guest_virt(vcpu, rsi + i * sizeof(index),
						&index, sizeof(index), &e);
			...
			if (!r) {
				q = kvm_write_guest_virt_system(vcpu,
					rdi + i * sizeof(index), &data,
					sizeof(index), &e);
				if (q != X86EMUL_CONTINUE) {
					return kvm_handle_memory_failure(vcpu,
									 q, &e);
				}
				...
				if (kvm_msr_user_space(vcpu, index,
					KVM_EXIT_X86_RDMSR, 0,
...
static int handle_help_wrmsrlist(int num, struct kvm_vcpu *vcpu)
{
	...
	for (i = num; i < 64; i++) {
		...
		if (rcx & (1 << i)) {
			q = kvm_read_guest_virt(vcpu, rdi + i * sizeof(data),
						&data, sizeof(data), &e);
			if (q != X86EMUL_CONTINUE)
			...
			if (!r) {
				rcx &= ~(1 << i); //bit[num] set zero
				kvm_rcx_write(vcpu, rcx);
			} else {
				if (kvm_msr_user_space(vcpu, index,
					KVM_EXIT_X86_WRMSR, data,
...
static int complete_userspace_rdmsrlist(struct kvm_vcpu *vcpu)
{
	...
	if (!vcpu->run->msr.error) {
		for (num = 0; num < 64; num++) {
			...
			if (rcx & (1 << num)) {
				p = kvm_read_guest_virt(vcpu,
						rsi + num * sizeof(value),
						&value, sizeof(value), &e);
				if (p != X86EMUL_CONTINUE) {
					return kvm_handle_memory_failure(vcpu,
									 p, &e);
				}
...
static int handle_wrmsrlist(struct kvm_vcpu *vcpu)
{
	...
	for (num = 0; num < 64; num++) {
		...
		if (rcx & (1 << num)) {
			p = kvm_read_guest_virt(vcpu, rsi + num * sizeof(index),
						&index, sizeof(index), &e);
			if (p != X86EMUL_CONTINUE)
				return kvm_handle_memory_failure(vcpu, p, &e);
			break;
...
#define cr4_fixed1_update(_cr4_mask, _reg, _cpuid_mask) do {		\
	...
	entry = kvm_find_cpuid_entry(vcpu, 0x7, 1);
