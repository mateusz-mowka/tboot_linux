...
static inline u64 tdh_mem_sept_rd(hpa_t tdr, gpa_t gpa, int level,
				  struct tdx_module_output *out)
{
	return kvm_seamcall(TDH_MEM_SEPT_RD, gpa | level, tdr, 0, 0, 0, 0, 0, 0, out);
}
...
static inline u64 tdh_export_track(hpa_t tdr,
				   ...
				   u64 mig_stream_info)
{
	return kvm_seamcall(TDH_EXPORT_TRACK, tdr, 0, mbmd_info, 0,
...
static inline u64 tdh_import_track(hpa_t tdr,
				    ...
				    u64 mig_stream_info)
{
	return kvm_seamcall(TDH_IMPORT_TRACK, tdr, 0, mbmd_info, 0,
			    mig_stream_info, 0, 0, 0, NULL);
}
...
{
	return kvm_seamcall(TDH_IMPORT_COMMIT, tdr, 0, 0, 0, 0, 0, 0, 0, NULL);
}
...
static inline u64 tdh_import_state_td(hpa_t tdr,
				      ...
				      struct tdx_module_output *out)
{
	return kvm_seamcall(TDH_IMPORT_STATE_TD, tdr, 0, mbmd_info,
			    buf_list_info, mig_stream_info, 0, 0, 0, out);
}
...
static inline u64 tdh_export_state_vp(hpa_t tdvpr,
				      ...
				      struct tdx_module_output *out)
{
	return kvm_seamcall(TDH_EXPORT_STATE_VP, tdvpr, 0, mbmd_info,
			    buf_list_info, mig_stream_info, 0, 0, 0, out);
}
...
static inline u64 tdh_import_state_vp(hpa_t tdvpr,
				      ...
				      struct tdx_module_output *out)
{
	return kvm_seamcall(TDH_IMPORT_STATE_VP, tdvpr, 0, mbmd_info,
			    buf_list_info, mig_stream_info, 0, 0, 0, out);
}
...
static inline u64 tdh_export_abort(hpa_t tdr,
				   ...
				   u64 mig_stream_info)
{
	return kvm_seamcall(TDH_EXPORT_ABORT, tdr, 0, mbmd_info,
