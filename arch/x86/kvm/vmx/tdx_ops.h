				   ...
				   hpa_t source, struct tdx_module_output *out)
{
	...
	return kvm_seamcall(TDH_MEM_PAGE_ADD, gpa | level,
			    tdr, hpa, source, out);
}
...
{
	tdx_clflush_page(hpa, tdx_sept_level_to_pg_level(level));
	return kvm_seamcall(TDH_MEM_PAGE_AUG, gpa | level, tdr, hpa, 0, out);
}
...
static inline u64 tdh_mem_page_demote(hpa_t tdr, gpa_t gpa, int level, hpa_t page,
				      struct tdx_module_output *out)
{
	return kvm_seamcall(TDH_MEM_PAGE_DEMOTE, gpa | level, tdr, page, 0, out);
}
