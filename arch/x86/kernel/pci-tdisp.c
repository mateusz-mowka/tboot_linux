...
static long tmgr_ioctl_msg_exchange(struct tdisp_mgr *tmgr, void __user *arg)
{
	...
	if (copy_from_user((void *)spdm_msg.req_addr,
			   (void *)user_msg.req_addr, user_msg.req_size)) {
		ret = -EFAULT;
		goto exit_free_all;
	}
	...
	if (copy_to_user((void *)user_msg.resp_addr,
			 (void *)spdm_msg.resp_addr, spdm_msg.resp_size)) {
		ret = -EINVAL;
		goto exit_free_all;
	}
...
static void tdisp_mgr_get_spdm(struct tdisp_mgr *tmgr)
{
	struct spdm_session *session = tmgr_to_sess(tmgr);

	spdm_session_msg_exchange_prepare(session);
...
static void tdisp_mgr_sess_keyupdate(struct work_struct *work)
{
...
static struct tdisp_mgr *tdisp_mgr_create(struct pci_tdisp_dev *tdev)
{
	...
	INIT_WORK(&tmgr->sess_keyupd_work, tdisp_mgr_sess_keyupdate);
...
static int tdisp_mgr_spdm_msg_exchange(struct spdm *spdm, struct spdm_message *msg)
{
	...
	return pci_doe_msg_exchange_sync(doe_mb, (void *)msg->req_addr,
					 (void *)msg->resp_addr, msg->resp_size);
}
					  ...
					  struct spdm_message *msg)
{
	...
	ret = pci_doe_msg_exchange_sync(tdev->doe_mb, (void *)msg->req_addr,
					(void *)msg->resp_addr, msg->resp_size);
	if (ret)
		return ret;
	...
	if (!work_pending(&tmgr->sess_keyupd_work))
		schedule_work(&tmgr->sess_keyupd_work);
...
static int tdisp_mgr_start(struct tdisp_mgr *tmgr)
{
	...
	/* By default TPA owns the SPDM and its session */
	tdisp_mgr_get_spdm(tmgr);
	tdisp_mgr_set_spdm_owner(tmgr, TMGR_SPDM_OWNER_USER);
