...
static int doe_send_req(struct uc_doe_mbox *mbox, struct uc_doe_transaction *t)
{
	...
	if (len > DOE_DATA_OBJ_MAX_LEN) {
		pr_err("Invalid DOE length (0x%lx exceeds max 0x%x) for cmd 0x%x\n",
...
struct uc_doe_mbox *uc_doe_create_mbox(phys_addr_t doe_addr)
{
	...
	if (!doe_enabled(mbox)) {
		pr_info("DOE mailbox is disabled\n");
		goto failed;
	}
...
int uc_doe_stage_ucode(struct uc_doe_mbox *mbox, void *uc, size_t uc_len)
{
	...
	if (uc_len & (sizeof(u32) - 1)) {
		pr_err("Invalid ucode length 0x%lx (non-dword aligned)\n", uc_len);
		return -EINVAL;
	}
	...
	while (req_len > 0) {
		...
		pr_debug("Ucode staging at offset 0x%x, length 0x%lx\n", offset, req_len);
