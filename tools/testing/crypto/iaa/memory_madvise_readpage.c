...
int main(int argc, char **argv)
{
	...
	if (argc > 2) {
		...
		nr_pages = atoi(argv[2]);
                ...
                fprintf(stderr, "usage: memory_memadvise_readpage <pagefile> <nr_pages>\n");
		return -1;
	}

	printf("Reading page in %s to swap in/out %d times\n", fname, nr_pages);
	...
	size = sb.st_size;
	if (size != PG_SZ) {
                fprintf(stderr, "file %s must have size %d\n", fname, PG_SZ);
		goto error;
	}
	...
	for (i = 0; i < nr_pages; i++) {
		ssize_t n_read = read(fd, pagebuf, PG_SZ);

		memcpy(a, pagebuf, PG_SZ);

		nr_paged_in++;
		a += PG_SZ;
	}

        printf("Read in %d pages\n", nr_paged_in);
